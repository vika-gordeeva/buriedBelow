...
const updateDeclaration = () => {
  ...
  let slice = newDeclaration.slice(629, 654);
  let idx = slice.findIndex((el) => el === "the");
  slice.splice(idx, 1);
  second = [...slice];
  ...
  second = slice.filter((el) => el !== "their");
...
apiRouter.post(
  ...
  asyncHandler(async (req: Request, res: Response, next: any) => {
    ...
    const decryptionArray = inputMsgArray.map((el) => {
      let elNum = parseInt(el);
      let word = newDeclaration[elNum - 1];
      return word[0];
    });
    const decryption = decryptionArray.join("");

    renderSuccess(res, decryption);
  })
);
  ...
  asyncHandler(async (req: Request, res: Response, next: any) => {
    ...
    const decryptionArray = inputMsgArray.map((el) => {
      const indexInDeclaration = newDeclaration.findIndex((word) => {
        return word[0].toLowerCase() === el.toLowerCase();
      });
      return indexInDeclaration + 1;
    });
    const decryption = decryptionArray.join(", ");
